...
# used to publish maps
#asily remder spatial objects from the sp and sf pacakges or dataframes with lat/long column
...
m%>% addProviderTiles(providers$Stamen) # to add third party maps
...
leaflet(cities) %>% addTiles() %>% addCircles(radius=~sqrt(Pop)*30, popup=~City)
...
# Markers
data(quakes) #locations of earthquakes on Fiji
head(quakes)

leaflet(data=quakes[1:20,]) %>% addTiles() %>% addMarkers(~long, ~lat, popup=~as.character(mag), label = ~as.character(mag))

leaflet(quakes) %>% addTiles() %>% addMarkers(clusterOptions = markerClusterOptions()) %>% addMeasure() %>% addMiniMap()
